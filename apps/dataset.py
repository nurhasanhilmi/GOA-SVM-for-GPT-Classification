...
def app():
    ...
    st.title('Dataset')
    st.write('The Guitar Playing Technique (GPT) dataset from the work of [Su et al. (2014)](http://mac.citi.sinica.edu.tw/GuitarTranscription/) was utilized.')

    st.header("Number of Sound Clips in GPT Dataset")
    st.write('This dataset comprises 7 playing techniques of the electrical guitar. ***(Total:', gpt.shape[0],' samples)***.')
    ...
    st.write('To represent musical signal, various audio descriptors such as: ***MFCC, $\Delta$MFCC (first-order derivative), $\Delta\Delta$MFCC (second-order derivative)*** was utilized which are computed using python package for music and audio analysis, [librosa](https://librosa.org/doc/latest/index.html).')
